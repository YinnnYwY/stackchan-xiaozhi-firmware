...
bool EspVideo::Capture() {
    ...
    // 不再自动把拍到的原始照片推到屏幕上显示——之前每次 self.camera.take_photo
    // (含 LLM 自己主动"看一眼"时)都会突然全屏闪出照片 5 秒,主人反馈"不要"。
    // 识别本身不受影响:frame_ 在上面已经拿到,Explain() 直接用它编码上传。
    // 用户主动要"给我看张图"走的是另一个工具 self.screen.preview_image,不受影响。
    return true;
}
